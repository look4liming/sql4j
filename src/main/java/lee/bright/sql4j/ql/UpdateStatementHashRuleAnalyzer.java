 ...
 * @author Bright Lee
 */
public final class UpdateStatementHashRuleAnalyzer {
	
	private Configuration configuration;
	private SourceCode sourceCode;
	private UpdateStatement updateStatement;
	
	public UpdateStatementHashRuleAnalyzer(
	...
	public void analyze() {
		...
		Map<String, String> hashColumnMap = new HashMap<String, String>(hashColumnSet.size());
		for (String hashColumnName : hashColumnSet) {
			hashColumnMap.put(hashColumnName, hashColumnName);
		}
		...
		for (int i = 0; i < setClauseList.size(); i++) {
			...
			String columnName = targetTableName + "." + updateTargetName;
			if (hashColumnMap.containsKey(columnName)) {
				throw Sql4jException.getSql4jException(sourceCode, updateTarget.getBeginIndex(), 
						"Hash columns cannot be updated.");
			}
		}
		BooleanValueExpression searchCondition = updateStatement.getSearchCondition();
		if (searchCondition == null) {
			throw Sql4jException.getSql4jException(sourceCode, updateStatement.getBeginIndex(), 
		...
		for (String hashColumnName : hashColumnMap1.keySet()) {
			...
			updateStatement.addHashColumnName(hashKeyValue.getKey());
			updateStatement.addHashColumnValue(hashKeyValue.getValue());
		}
	}
