 ...
 * @author Bright Lee
 */
public final class UpdateStatementSetFullyQualifiedColumnNamesAnalyzer {

	private Configuration configuration;
	private SourceCode sourceCode;
	private UpdateStatement updateStatement;
	
	public UpdateStatementSetFullyQualifiedColumnNamesAnalyzer(Configuration configuration, 
	...
	public void analyze() {
		...
		TableMetadata tableMetadata = configuration.getTableMetadata(targetTableName);
		...
		for (int i = 0; i < setClauseList.size(); i++) {
			...
			String updateTargetName = updateTarget.getContent().toLowerCase();
			if (!tableMetadata.hasColumnMetadata(updateTargetName)) {
				throw Sql4jException.getSql4jException(sourceCode, updateTarget.getBeginIndex(), 
						"This column does not exist in the table.");
			}
			NameChain fullyQualifiedName = getFullyQualifiedName(targetTable, updateTarget);
			updateTarget.setFullyQualifiedName(fullyQualifiedName);
		}
	...
	private void checkColumnInSearchCondition(NameChain column) {
		...
		NameChain fullyQualifiedName = getFullyQualifiedName(updateStatement.getTargetTable(), column);
		column.setFullyQualifiedName(fullyQualifiedName);
	}
	
	private static NameChain getFullyQualifiedName(NameChain targetTable, Name updateTarget) {
		List<Name> list = new ArrayList<Name>(targetTable.size() + 1);
		for (int i = 0; i < targetTable.size(); i++) {
			list.add(targetTable.get(i));
		}
		list.add(updateTarget);
